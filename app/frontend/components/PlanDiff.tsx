...
/**
 * Pending changes from `tofu plan`, grouped by module (Variant B of
 * Claude Design Session 2). Each row is one resource; expanding a row
 * reveals an inline tree-style diff of changed attributes.
 *
...
const ORDERED_KINDS: ActionKind[] = [
  "import",
  "create",
  "update",
  "replace",
...
export function PlanDiff() {
  ...
  // Re-group by module instead of action_kind.
        ...
        <div>
          {changes.map((c) => (
            <ChangeRow
              key={c.address}
              change={c}
              expanded={expanded.has(c.address)}
