...
| **Path shape** | `account/layer/component/` (e.g. `billing-account/infra/vpc`). `layer` is an open set (`infra`, `app`, `data`, …). **Environment == account** (account-per-env). |
...
devex-live/                              (real repo, later; live/devex-live/ in the POC)
  billing-account/
    infra/
      vpc/         ← leaf: *.tf, no child *.tf  → Spacelift stack "billing-account/infra/vpc"
      eks/
    app/
      payments-api/
  payments-account/
...
```
billing-account/infra/vpc/
  versions.tf      # required_providers (aws ~> 5)
  variables.tf     # aws_region, common_tags, use_localstack
  provider.tf      # minimal: region + default_tags + localstack dynamic endpoints
...
Blessed modules carry the mandatory scope/tags (happy path). Raw resources are
allowed in the same `main.tf` as the escape hatch, guarded by the writer +
checkov (per the architecture's hybrid decision).
...
- The existing **admin stack** enumerates deepest-`*.tf` folders under
  ...
  derives from the path (`billing-account/infra/vpc`).
...
Given `(account, layer, component)` the writer emits into
`devex-live/<account>/<layer>/<component>/`:
...
1. `live/devex-live/billing-account/infra/vpc/` — one reference leaf: boilerplate
   + a `module "vpc"` call sourced by **local path** to a catalog module.
...
- Wiring the app writer to emit `account/layer/component` paths — **Phase 2**
